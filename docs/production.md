...
```
java -jar backend/target/scala-2.12/triplan.jar
```
...
Triplan-based applications can be easily deployed to [Heroku](https://www.heroku.com).
...
Triplan already includes the Postgres driver and properly recognizes the `DATABASE_URL` environment variable that is set by Postgres Heroku.

Now you can deploy your app. Triplan includes an sbt task which will build the fat-jar and upload it:
