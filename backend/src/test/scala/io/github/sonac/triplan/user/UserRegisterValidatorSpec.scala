...
class UserRegisterValidatorSpec extends AnyFlatSpec with Matchers {
  "validate" should "accept valid data" in {
    val dataIsValid = UserRegisterValidator.validate("login", "admin@triplan.com", "password")
  ...
  "validate" should "not accept too short login" in {
    ...
    val dataIsValid   = UserRegisterValidator.validate(tooShortLogin, "admin@triplan.com", "password")
  ...
  "validate" should "not accept too short login after trimming" in {
    ...
    val dataIsValid            = UserRegisterValidator.validate(loginTooShortAfterTrim, "admin@triplan.com", "password")
  ...
  "validate" should "not accept password with empty spaces only" in {
    val dataIsValid = UserRegisterValidator.validate("login", "admin@triplan.com", "    ")
