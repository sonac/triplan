...
import io.github.sonac.triplan.passwordreset.PasswordResetApi.{ForgotPassword_IN, ForgotPassword_OUT, PasswordReset_IN, PasswordReset_OUT}
...
class PasswordResetApiTest extends BaseTest with TestEmbeddedPostgres with Eventually {
  ...
  def codeSentToEmail(email: String): String = {
    ...
    val emailData = DummyEmailSender
      .findSentEmail(email, "SoftwareMill Triplan password reset")
